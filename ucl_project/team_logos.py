...
def get_best_team_logo(team: object, size: tuple[int, int] = (32, 32)) -> ctk.CTkImage:
    ...
    espn_path = _local_logo_path(ESPN_LOGO_DIR, _get_team_value(team, "espn_id"))
    if espn_path is not None:
        return _load_local_logo(espn_path, size)

    transfermarkt_path = _local_logo_path(TRANSFERMARKT_LOGO_DIR, _get_team_value(team, "transfermarkt_id"))
